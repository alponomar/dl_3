...
class Siamese(object):
    ...
    def inference(self, x, reuse = False):
        ...
        with tf.variable_scope('ConvNet', reuse=reuse) as conv_scope:
            ...
            def _forward_conv_layer(name, w_shape, b_shape, x_inp, max_pool_kernel, max_pool_stride, act_func, reuse):
            ...
            conv1 = _forward_conv_layer(name='conv1', w_shape=[5, 5, 3, 64], b_shape=64, 
              x_inp=x_inp, max_pool_kernel=3, max_pool_stride=2, act_func=tf.nn.relu, reuse=reuse)  
            conv2 = _forward_conv_layer(name='conv2', w_shape=[5, 5, 64, 64], b_shape=64, 
              x_inp=conv1, max_pool_kernel=3, max_pool_stride=2, act_func=tf.nn.relu, reuse=reuse) 
